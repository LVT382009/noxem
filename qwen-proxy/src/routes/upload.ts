  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    if (!response.ok) {
      ...
      if (response.status === 401 && attempt === 0) {
        ...
        const refreshed = await refreshUploadHeaders(accountId);
        if (refreshed) {
 ...
 */
async function refreshUploadHeaders(accountId?: string): Promise<Record<string, string> | null> {
  try {
    const { headers: qHeaders } = await getQwenHeaders(true, accountId);
...
export async function uploadFile(c: Context) {
  try {
    ...
    // Get full Qwen headers with bx-ua/bx-umidtoken. Thread accountId: the
    // default "global" namespace has no Playwright context, only real accounts do
    // (same pattern stream-creator uses).
      ...
      if (mediaUrl.startsWith("http://") || mediaUrl.startsWith("https://")) {
        ...
        try {
          ...
          const detectedExt =
            ...
            (isVideoData ? "mp4" : isAudioData ? "mp3" : isTextData ? "txt" : "png");
          const base64Data = mediaUrl.split(",")[1];
          const buffer = Buffer.from(base64Data, "base64");
          filename = `${isVideoData ? "video" : isAudioData ? "audio" : "file"}_${Date.now()}.${detectedExt}`;
          if (isTextData) {
            // Text-class files (txt/md/csv/json/xml/html): the OSS file_url path
            // does NOT resolve correctly for text on Qwen's backend (the model
            // receives unrelated captured telemetry instead of the file content).
            // Inline-inject the decoded text into the message instead — verified
            // clean, sidesteps STS/OSS entirely, no files[] entry needed.
