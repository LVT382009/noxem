...
export async function createQwenStream(
      ...
      if (largePromptFile) {
        ...
        if (largePromptFile.name.endsWith('.txt') || largePromptFile.name.endsWith('.log') || largePromptFile.name.endsWith('.md')) {
          try {
            ...
            console.warn(`[Qwen] Error reading uploaded file content:`, fileError instanceof Error ? fileError.message : String(fileError));
  ...
  if (pendingMultimodal && pendingMultimodal.length > 0) {
    try {
      ...
      // r.text carries inline-injected text for text-class file attachments
      // (txt/md/csv/json/xml/html) — processImagesForQwen decodes those instead
      // of the broken OSS path. binary media still come through r.files.
      const inlineFileText: string[] = [];
      for (const r of results) {
        resolvedFiles.push(...r.files);
        if (r.text) inlineFileText.push(r.text);
      }
      if (inlineFileText.length > 0) {
        finalPrompt = (finalPrompt || '') + inlineFileText.join('\n');
      }
    } catch (err: any) {
...
/**
 * createContinuationStream — issue a follow-up completion turn to an EXISTING
 * Qwen chat (same chatId) with parent_id = the previous round's last
 * response_id. Used by AutoContinue (stream-handler.ts) when a round hit
 * Qwen's per-response output cap, so the model resumes from the same
 * conversation and the client sees one uninterrupted answer instead of a
 * truncation.
 *
 * Differs from createQwenStream: it does NOT lease a new warm chat (it reuses
 * the chatId + anti-bot headers already allocated for round 1), and it skips
 * large-prompt upload + multimodal processing (the continuation prompt is a
 * short instruction; file content already lives in the chat). Requires the
 * account's Qwen browser page to still be alive.
 */
export async function createContinuationStream(args: {
  chatId: string;
  ...
  const { chatId, chatHeaders, accountId, prompt, modelId, parentId, enableThinking } = args;

  const page = getPageForAccount(accountId);
  if (!page || page.isClosed() || !page.url().includes('chat.qwen.ai')) {
    throw new Error(`Cannot continue Qwen chat ${chatId}: browser page for ${accountId} is not active.`);
  }
  ...
  const payloadSize = Buffer.byteLength(payloadJson);
  if (payloadSize > MAX_PAYLOAD_SIZE) {
    throw new Error(`Continuation payload too large: ${payloadSize} bytes exceeds ${MAX_PAYLOAD_SIZE} bytes`);
  }
  const payloadMB = payloadSize / (1024 * 1024);
  const timeoutMs = BASE_TIMEOUT_MS + Math.ceil(payloadMB * TIMEOUT_PER_MB);
  ...
  if (browserResult.contentType.includes('text/event-stream') && browserResult.status < 400) {
    const controller = new AbortController();
    const stream = addIdleTimeoutToStream(
      browserResult.stream,
      controller,
      timeoutMs,
      `Qwen continuation ${chatId}`,
      () => { try { browserResult.abort(); } catch { /* ignore abort race */ } }
    );
    return { stream, controller, uiSessionId: chatId };
  }

  // Non-stream or error response: surface so the continuation loop aborts
  // gracefully and keeps the content already streamed to the client.
  if (browserResult.body) {
    handleErrorBody(browserResult.body, browserResult.status);
  }
  throw new Error(`Continuation stream for chat ${chatId} returned non-stream response: ${browserResult.status} ${browserResult.statusText}`);
}