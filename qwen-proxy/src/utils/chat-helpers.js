...
const parserMessages = async (messages, thinking_config, chat_type) => {
  ...
  // Qwen2API pattern: do NOT extract system messages separately.
  // System messages are folded into history as "system:content" via
  // formatHistoryMessages, just like user/assistant messages.
  // System text + tool prompt are prepended to the LAST user message
  // by the calling route AFTER parserMessages runs (Qwen2API pattern).
  // Previously, extracting system messages and prepending them at the
  // top of combinedText created a 30K-char prefix that caused Qwen
  // upstream to CLOSE the connection at ~8-12s.
  ...
  // Build combined text: history (with system folded in) -> last message
  let combinedText = ""
  if (historyText) {
