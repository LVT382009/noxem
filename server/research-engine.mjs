...
// Extract the outermost JSON array from text (greedy match + bracket counting)
  ...
  for (let i = start; i < text.length; i++) {
    if (text[i] === '[') depth++;
      ...
      if (depth === 0) {
        ...
        // If greedy match failed, try next '['
        continue;
      }
    }
  }
...
// Sanitize user input before LLM interpolation to prevent prompt injection
function _sanitizeUserInput(text) {
  let s = (text || '').substring(0, 500);
  // Strip newlines and carriage returns
  s = s.replace(/\n/g, " ").replace(/\r/g, " ");
  // Strip format keywords that could override LLM instructions
  s = s.replace(/TOPIC:|QUERIES:|ENTITY:/gi, '[FILTERED]');
  return s;
}
...
export function triggerResearch({ sessionId, userMessage, assistantResponse, storeMemoryFn, embedFn, isEmbeddingReadyFn }) {
  // Atomic check-and-set: prevent duplicate pipelines for same session
  if (sessionRunning.has(sessionId)) return;
  const lastTime = sessionLastResearch.get(sessionId) || 0;
  if (Date.now() - lastTime < RESEARCH_MIN_INTERVAL_MS) return;
  const count = sessionResearchCount.get(sessionId) || 0;
  if (count >= RESEARCH_MAX_TOPICS_PER_SESSION) return;
...
Assistant: ${_sanitizeUserInput(assistantResponse)}`,
  ...
  const queriesMatch = llmResponse.match(/QUERIES:\s*(.+)/i);
  ...
  if (!subQueries.length) {
    const q = `how to ${topic.toLowerCase()} ${getCurrentYear()}`;
    subQueries = [q, `${topic} best practices ${getCurrentYear()}`, `${topic} common errors`];
  }
  // Limit sub-queries
...
Example: [2, 4] means facts at index 2 and 4 are contradicted.`,
  ...
  try {
    ...
    const removeIndices = new Set(parsed.filter(n => Number.isInteger(n) && n >= 0 && n < facts.length));
...
Merge related facts. Keep distinct facts separate. Limit to 3-5 synthesized facts.`,
  ...
  if (!result) return facts;
  try {
    const synthesized = extractJsonArray(result);
    if (!synthesized) return facts;
...
function regexTopicDetection(text) {
  ...
  for (const { pattern, type } of techPatterns) {
    ...
    if (match) {
      ...
      switch (type) {
        case 'build':
          result.searchQuery = `how to build ${topicText} ${getCurrentYear()}`;
          result.subQueries = [`how to build ${topicText} ${getCurrentYear()}`, `${topicText} best practices ${getCurrentYear()}`, `${topicText} common errors`];
          break;
        case 'howto':
          result.searchQuery = `${topicText} ${getCurrentYear()}`;
          result.subQueries = [`${topicText} ${getCurrentYear()}`, `${topicText} tutorial`, `${topicText} troubleshooting`];
          break;
        case 'fix':
          result.searchQuery = `how to fix ${topicText} ${getCurrentYear()}`;
          result.subQueries = [`how to fix ${topicText} ${getCurrentYear()}`, `${topicText} root cause`, `${topicText} workaround`];
          break;
        case 'error':
        ...
        default:
          result.searchQuery = `${topicText} ${getCurrentYear()}`;
          result.subQueries = [`${topicText} ${getCurrentYear()}`];
      }
      break;
    }
  }
...
${searchContext}`;
  ...
  if (!response) return [];
  try {
    const facts = extractJsonArray(response);
    if (!facts) return [];
...
setInterval(() => {
  ...
  evictOldestEntries(sessionLastResearch, MAP_MAX_SIZE);
  evictOldestEntries(sessionResearchCount, MAP_MAX_SIZE);
  evictOldestEntries(recentResearchTopics, MAP_MAX_SIZE);
}, 300_000).unref();
