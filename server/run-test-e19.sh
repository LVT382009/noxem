#!/usr/bin/env bash
# run-test-e19.sh — E19 D1 contradiction flip + resolve regression.
...
# Clear the REAL db (PROJECT_ROOT/data) AND the server-relative data dir — rows must not leak across
...
node test_e19_contradiction.mjs
