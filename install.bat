...
echo === Hermes Noxem Memory Provider - Windows Installer ===
echo.
...
wsl -d Ubuntu -e bash -c "mkdir -p $HOME/.hermes/plugins/memory/noxem && cp '%WSL_DIR%plugins/memory/noxem/'* $HOME/.hermes/plugins/memory/noxem/"
...
wsl -d Ubuntu -e bash -c "cat > '%WSL_DIR%start.bat' << 'BAT'
@echo off
wsl -d Ubuntu -e bash -c \"cd '%WSL_DIR%server' && ENABLE_EMBEDDING=true ENABLE_ADVISOR=true ENABLE_MAINTENANCE=true exec node memory-server.mjs\"
pause
BAT"
